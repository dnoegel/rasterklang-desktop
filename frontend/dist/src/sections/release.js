...
export function mount(host, ctx, params = {}) {
  ...
  if (!release) {
    host.append(el("div", { class: "empty-state" }, "Release not found."));
    return;
  }
  ...
  host.append(el("section", { class: "detail-hero" }, [
    ...
    el("div", { class: "detail-hero__copy" }, [
      ...
      el("div", { class: "pill-row" }, [
        ...
        pill("Time", release.duration ? `${Math.round(release.duration / 60)} min` : "--"),
        pill("Source", typeLabel(release.type)),
      ]),
      el("div", { class: "hero-actions" }, [
        el("button", { class: "btn btn--accent", onclick: () => tracks[0] && playTrack(ctx, tracks[0], tracks) }, [svg("play", 16), "Play"]),
        el("button", { class: "btn btn--ghost", onclick: () => ctx.router.navigate("artist", { id: release.artistId }) }, [svg("artist", 16), "Profile"]),
      ]),
    ]),
  ]));
